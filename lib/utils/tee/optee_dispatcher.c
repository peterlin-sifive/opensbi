 ...
 *
 * This context holds only the OP-TEE domain reference.
 ...
 * mpxy_reqfwd_find_channel_by_hartid(). The device tree binding guarantees
 * that reqfwd nodes are siblings of TEE nodes under the same CPU, so no
 * explicit relationship verification is needed.
 */
